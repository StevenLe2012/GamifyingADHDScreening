...
namespace UIElements
{
    public class DialogueUI : MonoBehaviour
    {
        ...
        // >>> Hami - ADD: convenience method to bind a DialogueUnit
        ...
        // >>> Hami - CHANGED: now sets text color based on DialogueUnit.character
        private void DisplaySentence(AudioObjects audioObject)
        {
            if (!string.IsNullOrEmpty(audioObject.character))
            {
                if (string.Equals(audioObject.character, playerCharacterName, System.StringComparison.OrdinalIgnoreCase))
                    sentenceText.color = playerColor;   // PC line
                else
                    sentenceText.color = npcColor;      // NPC line
            }
            else
            {
                sentenceText.color = systemColor;       // fallback
            }
        ...
        // >>> CHANGED: player dialogue options always yellow
        private void DisplayDialogueOptions()
        {
            var optionsCount = _dialogueOptions.Length;
            for (var i = 0; i < _buttons.Length; i++)
